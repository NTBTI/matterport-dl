...
def patchGetModelDetails():
    localServer = f"http://127.0.0.1:8080/{pageId}"
...
class OurSimpleHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        getModelId = os.path.basename(self.path)
        
        if getModelId.startswith("?m="):
           modelId = getModelId.split("?m=")[-1].split("&")[0]
           print(f"Model ID: {modelId}")

        if self.path.startswith(f"/api"):
            redirect_msg = f"adding pageid to {self.path}"
            x= f"{modelId}/{self.path}"
            self.path = x

        if self.path.startswith(f"/{modelId}/js/showcase.js") and os.path.exists(f"{modelId}/js/{SHOWCASE_INTERNAL_NAME}"):
            redirect_msg = "using our internal showcase.js file"
            self.path = f"{modelId}/js/{SHOWCASE_INTERNAL_NAME}"

        if self.path.startswith(f"/{modelId}/locale/messages/strings_") and not os.path.exists(f".{self.path}"):
            redirect_msg = "original request was for a locale we do not have downloaded"
            self.path = f"/{modelId}/locale/strings.json"
    ...
    def do_POST(self):
        post_msg = None
        try:
            if self.path == f"/{modelId}/api/mp/models/graph":
                ...
                if option_name in GRAPH_DATA_REQ:
                    file_path = f"{modelId}/api/mp/models/graph_{option_name}.json"
...
if __name__ == "__main__":
    ...
    customDownloadsDir = getCommandLineArg("--base-folder", True)
    if customDownloadsDir:
      DownloadsDir = customDownloadsDir
    ...
    if len(sys.argv) == 2: # Download mode
        ...
        if os.path.isdir(pageId) and not os.path.isdir(os.path.join(DownloadsDir, pageId)):
          print("WARNING:")
          print("The folder '" + pageId + "' does not exist in the '" + DownloadsDir + "' directory, but does exist in the root directory.")
          print("If you recently updated matterport-dl, then please move the '" + pageId + "' folder into the '" + DownloadsDir + "' directory")
